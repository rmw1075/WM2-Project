...
<div id="quizHolder" style="margin-top:12em; margin-left:1em;">
    ...
    if ($_SESSION["quiz"] == true) {
        ...
        echo "<form name=\"quizForm\" method=\"POST\" action=\"../php/grading.php\" onsubmit=\"return validateQuiz();\"><br />";
        $optionsOrder = array();
        for ($a = 0; $a < count($questions); $a++) {
            ...
            $num = eval('return ' . $a . "+" . '1' . ';');
            echo "<fieldset class=\"fset\"  id=\"fset$a\" name=\"fset$a\">";
            echo "<legend>" . "$num:" . " $questionName" . "</legend><br />";
            for ($b = 0; $b < 4; $b++) {
                echo "<input type=\"radio\" id=\"option$a\" name=\"option$a\" value= \"$options[$b]\">";
                echo "$options[$b]";
                echo "</input><br />";
            }
